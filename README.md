...
- **🗑 管理端记录保留策略**：admin 保留全部记录，但只能删除存放超过 7 日的日志或清零该人的 x；
- **↩️ 误判撤回**：被警告消息备案页可一键撤回误判警告，自动递减 count 并标记消息以免再次误判，同时向云端发送撤回请求（需 bot_id 一致）；
- **📨 私聊申诉系统**：用户私聊机器人可自然语言查询警告记录、申请撤回警告。LLM 复核理由后自动撤回/拒绝/转管理员审核，撤回时自动跨群解禁并同步云端；
- **🖥️ 云端可视化管理器**：浏览器访问服务端地址即可进入管理后台，查看记录、备案、特殊记录、审计日志、请求日志；
...
- 📨 **私聊申诉系统**：用户私聊机器人可自然语言查询警告记录、申请撤回警告。LLM 意图识别（查询/申诉/闲聊）→ LLM 复核理由（充分自动撤回/不足拒绝/难评判转管理员）→ 撤回时跨群解禁 + 云同步。管理员可通过群指令或插件页 Web 审核申诉。
...
- 🗑 **管理端记录保留策略**：admin 保留全部记录，但只能删除存放超过 7 日的备案日志或清零该人的 x（清零通过 `admin_rev` 强制下发）。
...
10. **后台任务**：
    - 每 12 小时将所有人的 x - 1（不低于 0），停机期间会补算；
    - 每 6 小时检查是否有超时备案记录需要归档；
    - 每日生成一次统计总结。

### x 的含义与衰减

- **x（count）**：当前累计警告次数，每 12 小时 -1，触发禁言时按此计算时长。
- **total**：历史总警告次数，永不衰减，用于审计。
- 例：用户第 6 次被警告（x=6），`mute_multiplier=10` → 禁言 60 分钟；12 小时后 x 衰减为 5，不再处于禁言阈值之上。
...
- **用户名可点击**，弹出该用户的全部警告详情弹窗（时间、消息原文、判定原因、x 值、禁言状态、撤回标记）；
...
| POST | `cloud/delete_old_logs` | Dashboard | 删除存放超过 N 天的备案日志（body: `{"days": 7}`，需 admin_token） |
| GET | `appeals` | Dashboard | 获取申诉列表（支持 status 过滤：pending_review/approved/rejected/auto_revoked） |
| POST | `appeal/approve` | Dashboard | 通过申诉（body: `{"appeal_id": "..."}`，撤回警告 + 云同步） |
| POST | `appeal/reject` | Dashboard | 拒绝申诉（body: `{"appeal_id": "..."}`） |
...
3. **📋 警告次数统计**：搜索（UID/用户名/平台）、批量删除（复选框）、单条删除、批量清零、单条清零；
   - **用户名可点击**，弹出该用户全部警告详情弹窗；
4. **📄 备案**：展示每次警告的句子与原因（时间、用户、平台、消息、原因、x 值、来源 bot），支持搜索、按用户过滤；
   - 管理员可删除存放超过 7 天的备案日志；
   - **用户名可点击**，弹出该用户全部警告详情弹窗；
5. **🔴 特殊记录**：按人分类查看政治敏感/违法内容；
6. **📝 审计日志**：查看最近 500 条审计记录（撤回/删除/登录/上传/同步/清零/衰减等操作）；
7. **🌐 请求日志**：查看最近 500 条 HTTP 请求记录（方法/路径/状态/客户端）；
8. **🚫 IP 黑名单**（仅管理员）：添加/移除 IP 黑名单，被拉黑 IP 无法访问服务。

> client_token 登录后自动隐藏除「概览/警告次数统计/备案/特殊记录」外的标签页与所有操作按钮，仅可查看与刷新。
...
| `/appeal_list` | `申诉列表` | 查看待审核申诉列表 |
| `/appeal_approve <申诉编号>` | `申诉通过` | 通过申诉，撤回对应警告并跨群解禁 |
| `/appeal_reject <申诉编号>` | `申诉拒绝` | 拒绝申诉 |

...
- **📨 私聊申诉系统**：用户可私聊机器人，通过自然语言查询自己的警告/禁言记录并申请撤回警告。
  - **LLM 意图识别**：私聊消息先由 LLM 判断意图（查询记录 / 申请撤回 / 闲聊），闲聊消息继续走恶意检测流程；
  - **查询记录**：用户私聊询问时可查看自己的当前警告次数、历史累计、禁言状态、最近 5 条警告详情；
  - **LLM 复核撤回**：申请撤回时由 LLM 复核理由，自动选择最相关的警告记录：
    - 理由充分 → 自动撤回警告 + 跨群解禁 + 云同步；
    - 理由不足 → 拒绝并告知原因；
    - 难以评判 → 转管理员人工审核；
  - **管理员审核**：难以评判的申诉转发到管理群和管理员 QQ，管理员可通过：
    - 群内指令：`/申诉通过 <id>`、`/申诉拒绝 <id>`、`/申诉列表`
    - 插件页 Web：「申诉审核」标签页可视化操作
  - **跨群解禁**：撤回警告时自动在机器人是管理员的群里解除该用户的禁言（`set_group_ban duration=0`），重置本地禁言状态；
  - **云同步**：申诉撤回自动同步至云端（复用 `_cloud_revoke_record`，校验 bot_id 在 sources 中）；
  - **持久化**：申诉记录持久化到 `warning_data.json` 的 `appeals` 字段，上限 2000 条；

### 新增配置项

| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `enable_appeal` | bool | true | 启用私聊申诉功能 |
| `appeal_admin_groups` | list | [] | 申诉审核通知管理群列表 |
| `appeal_admin_users` | list | [] | 申诉审核通知管理员 QQ 列表 |
| `appeal_auto_revoke` | bool | true | LLM 判定理由充分时自动撤回 |
| `appeal_pending_timeout_hours` | int | 72 | 待审核申诉超时时间（小时） |

### 新增指令

| 指令 | 别名 | 权限 | 说明 |
|------|------|------|------|
| `appeal_list` | `申诉列表` | 管理员 | 查看待审核申诉 |
| `appeal_approve` | `申诉通过` | 管理员 | 通过申诉并撤回警告 |
| `appeal_reject` | `申诉拒绝` | 管理员 | 拒绝申诉 |

### 新增 Web API

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `appeals` | 获取申诉列表（支持 status 过滤） |
| POST | `appeal/approve` | 通过申诉 |
| POST | `appeal/reject` | 拒绝申诉 |

### 变更

- 衰减间隔从 2 小时改为 12 小时（`DECAY_INTERVAL = 12 * 3600`）；
- 合法衰减改为按 bot_id 统一衰减模式（客户端不传 keys，服务端遍历 sources 匹配的记录）。
